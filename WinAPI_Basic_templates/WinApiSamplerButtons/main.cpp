...
LRESULT OnButtonClick1(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
...
LRESULT OnButtonClick2(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	static auto buttonClicked = 0;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == button1) {
		return OnButtonClick1(hWnd, message, wParam, lParam);
	}
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == button2) {
		return OnButtonClick2(hWnd, message, wParam, lParam);
	}
...
int main() {
	window = CreateWindowEx(0, WC_DIALOG, L"Button example", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 406, 316, nullptr, nullptr, nullptr, nullptr);
	...
	staticText1 = CreateWindowEx(0, WC_STATIC, L"button1 clicked 0 time", WS_CHILD | WS_VISIBLE, 50, 200, 200, 23, window, nullptr, nullptr, nullptr);
	staticText2 = CreateWindowEx(0, WC_STATIC, L"button2 clicked 0 time", WS_CHILD | WS_VISIBLE, 50, 230, 200, 23, window, nullptr, nullptr, nullptr);
