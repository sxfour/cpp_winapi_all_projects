...
wstring findWhat = L"sxfour";
wstring replaceWith = L"sxfour51";
...
UINT_PTR OnReplace(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if ((findReplace->Flags & FR_REPLACE) == FR_REPLACE) {
		OutputDebugString((L"Replace \""s + findReplace->lpstrFindWhat + L"\" with \""s + findReplace->lpstrReplaceWith + L"\"\n"s).c_str());
	}
	if ((findReplace->Flags & FR_REPLACEALL) == FR_REPLACEALL) {
		OutputDebugString((L"Replace all \""s + findReplace->lpstrFindWhat + L"\" with \""s + findReplace->lpstrReplaceWith + L"\"\n"s).c_str());
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (message == UM_REPLACE_BUTTON_CLICKED && hWnd == window) {
		return OnReplace(hWnd, message, wParam, lParam);
	}
...
int main() {
	window = CreateWindowEx(0, WC_DIALOG, L"Replace Dialog example", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 300, 300, nullptr, nullptr, nullptr, nullptr);
	button1 = CreateWindowEx(0, WC_BUTTON, L"Replace...", WS_CHILD | WS_VISIBLE, 10, 10, 100, 25, window, nullptr, nullptr, nullptr);
