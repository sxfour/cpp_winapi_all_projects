...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == checkBox1) {
		return OnCheckBox1Click(hWnd, message, wParam, lParam);
	}
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == checkBox2) {
		return OnAutoCheckBoxClick(hWnd, message, wParam, lParam);
	}
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == checkBox3) {
		return OnAutoCheckBoxClick(hWnd, message, wParam, lParam);
	}
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == checkBox4) {
		return OnAutoCheckBoxClick(hWnd, message, wParam, lParam);
	}
	if (message == WM_COMMAND && HIWORD(wParam) == BN_CLICKED && reinterpret_cast<HWND>(lParam) == checkBox5) {
		return OnCheckBox5Click(hWnd, message, wParam, lParam);
	}
...
int main() {
	...
	checkBox4 = CreateWindowEx(0, WC_BUTTON, L"Checked", WS_CHILD | BS_AUTO3STATE | WS_VISIBLE, 30, 120, 104, 24, window, nullptr, nullptr, nullptr);
	checkBox5 = CreateWindowEx(0, WC_BUTTON, L"Unchecked", WS_CHILD | BS_CHECKBOX | WS_VISIBLE, 30, 150, 104, 24, window, nullptr, nullptr, nullptr);
