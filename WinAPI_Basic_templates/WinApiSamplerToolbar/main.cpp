...
LRESULT OnToolBarItemClick(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	static map<IdMenu, std::wstring> items = { {IdMenu::New, L"Создать"}, {IdMenu::Open, L"Открыть"}, {IdMenu::Save, L"Сохранить"}, {IdMenu::Cut, L"Вырезать"}, {IdMenu::Copy, L"Копировать"}, {IdMenu::Paste, L"Вставить"}, {IdMenu::Print, L"Печать"}, {IdMenu::About, L"О программе"} };
	auto it = items.find(static_cast<IdMenu>(LOWORD(wParam)));

	AppendLine(textBox1, (it != items.end() ? it->second : L"(unknown item)"s) + L" = Сигнал исполнен"s);
...
int main() {
	...
	vector<TBBUTTON>toolbarButtons{
		{ MAKELONG(STD_FILENEW, ImageListID), static_cast<int32_t>(IdMenu::New), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Создать"},
		{ MAKELONG(STD_FILEOPEN, ImageListID), static_cast<int32_t>(IdMenu::Open), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Открыть..."},
		{ MAKELONG(STD_FILESAVE, ImageListID), static_cast<int32_t>(IdMenu::Save), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Сохранить"},
		{ MAKELONG(0, ImageListID), static_cast<int32_t>(IdMenu::Separator1), 0, BTNS_SEP, {0}, 0, (INT_PTR)L""},
		{ MAKELONG(STD_CUT, ImageListID), static_cast<int32_t>(IdMenu::Cut), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Вырезать"},
		{ MAKELONG(STD_COPY, ImageListID), static_cast<int32_t>(IdMenu::Copy), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Копировать"},
		{ MAKELONG(STD_PASTE, ImageListID), static_cast<int32_t>(IdMenu::Paste), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Вставить"},
		{ MAKELONG(0, ImageListID), static_cast<int32_t>(IdMenu::Separator2), 0, BTNS_SEP, {0}, 0, (INT_PTR)L""},
		{ MAKELONG(STD_PRINT, ImageListID), static_cast<int32_t>(IdMenu::Print), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&Печать"},
		{ MAKELONG(0, ImageListID), static_cast<int32_t>(IdMenu::Separator3), 0, BTNS_SEP, {0}, 0, (INT_PTR)L""},
		{ MAKELONG(STD_HELP, ImageListID), static_cast<int32_t>(IdMenu::About), TBSTATE_ENABLED, BTNS_AUTOSIZE, {0}, 0, (INT_PTR)L"&О программе..."},
	};
